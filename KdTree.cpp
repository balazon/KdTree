...
void KdTree::updateNeighbours()
{
	for(int i = 0; i < pointNodes.size(); i++)
	{
		int currentBest[MAXNEARBY];
		float currentBestDistancesSq[MAXNEARBY];
		//std::fill_n(currentBest, MAXNEARBY, -1);
		PointNode* pn = &pointNodes[i];
		int nearbyCount = 0;
		findNearby(pn->p, root, currentBest, currentBestDistancesSq, nearbyCount, 0);
		std::cout << "owner " << pn->p->pointId << " neighbours: ";
		for(int j = 0; j < nearbyCount; j++)
		{
			setNeighbours(pn->p->pointId, currentBest[j]);
		}
		std::cout << "\n\n";
	}
}
...
void KdTree::insertPoint(Point* searchingPoint, int currentBest[], float currentBestDistancesSq[], int& nearbyCount, Point* nearbyPoint)
{
	...
	int upper = nearbyCount + 1 > MAXNEARBY ? MAXNEARBY : nearbyCount + 1;
